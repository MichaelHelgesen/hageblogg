...
import { PAGE_BY_SLUG_QUERY, PLANT_CATEGORY_QUERY } from "@/sanity/lib/queries";
...
export async function generateMetadata(
  ...
  const page = await client.fetch(PAGE_BY_SLUG_QUERY, { slug: "planter" });
...
export default async function PlantPage() {
  const [page, posts] = await Promise.all([
    client.fetch(PAGE_BY_SLUG_QUERY, { slug: "planter" }),
    client.fetch(PLANT_CATEGORY_QUERY),
  ]);
    ...
    <section className="py-16">
      <Container>
        ...
        <div className="mt-12 grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {posts.sort((a,b) => a.title.localeCompare(b.title, "nb")).map((post) => (
            <article
              key={post._id}
